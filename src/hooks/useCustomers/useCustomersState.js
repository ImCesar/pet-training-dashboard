...
export const useCustomersState = customers => {
  ...
  useEffect(() => {
    ...
    const parsedStoredCustomers = storedCustomersExist && JSON.parse(storedCustomers);

    if (parsedStoredCustomers) {
      const storedCustomersWithDate = parsedStoredCustomers.map(customer => { return { ...customer, joinDate: new Date(customer.joinDate) }});
  ...
  return { customers: state.customers, findCustomerById, initCustomers, updateCustomer };
}